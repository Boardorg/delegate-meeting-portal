...
const REV_TIERS = ['<10M', '10M-50M', '50M-100M', '100M-500M', '500M-1B', '1B-5B', '>5B'];

const SORT_ORDER: Record<string, string[]> = {
    annualRevenue: REV_TIERS,
    budgetaryResponsibility: ['<1M', '1M-10M', '10M-50M', '50M-100M', '100M-500M', '500M-1B', '>1B'],
    plannedSpend: ['<1M', '1M-5M', '5M-25M', '25M-100M', '>100M'],
    companySize: ['1-50', '51-200', '200-500', '500-1000', '1000-5000', '>5000'],
};

const LIST_BREAK = 860;
const LIST_COLS = '140px 110px 72px 82px 72px 68px 1fr 1fr 1fr 1fr';
const LIST_HEADERS = ['Name / Title', 'Company', 'Revenue', 'Budget Resp.', 'Planned Spend', 'Co. Size', 'Specialization', 'Industries', 'Regions', 'Priorities'];
...
function CardMoreToggle({ p }: { p: AttendeeProfile }) {
    ...
    return (
        <div className={`card-more ${open ? 'open' : ''}`}>
            <button className="card-more-trigger" onClick={() => setOpen(o => !o)}>
            ...
            {open && (
                <div className="card-more-body">
                    {groups.map(g => (
                        <div key={g.label}>
                            <div className="card-more-group-label">{g.label}</div>
                            <div className="card-more-group-val">{g.items.join(' · ')}</div>
                        </div>
                    ))}
                </div>
            )}
    ...
    return (
        <div className="d-item">
            <div className="d-item-hero">
                ...
                <button className="d-remove" onClick={() => onRemove(d.id)} title="Remove">
                    ✕
                </button>
            </div>
            <button
            ...
            <div className={`d-detail-body ${detailOpen ? 'open' : ''}`}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', fontSize: '11px', alignItems: 'center' }}>
                    <span style={{ color: 'var(--t3)', flexShrink: 0 }}>Revenue</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', fontSize: '11px' }}>
                    ...
                    <span style={{ color: 'var(--t2)', textAlign: 'right' }}>{str(p.companySize)}</span>
                </div>
...
function FiltersPanel({
    ...
    onApplyFilter: (id: string, val: string, checked: boolean, type: 'single' | 'multi') => void;
}) {
    return (
        <>
            {filterConfig.map(f => {
                ...
                return (
                    <div key={f.id} className={`acc-item ${isOpen ? 'open' : ''}`}>
                        <button className="acc-trigger" onClick={() => onToggleAccordion(f.id)}>
                            <span className="acc-arrow" />
                            <span className="acc-label">{f.label}</span>
                            {count > 0 && <span className="acc-badge-cnt">{count}</span>}
                        </button>
                            ...
                            {f.options.map(opt => (
                                <label key={opt} className="filter-opt">
                                    <input
                                        ...
                                        name={`${prefix}-f-${f.id}`}
                                        value={opt}
                                        checked={(activeFilters[f.id] || []).includes(opt)}
                                        onChange={e =>
                                            onApplyFilter(f.id, opt, e.target.checked, f.type)
                                        }
                                    />
                                    {opt}
                                </label>
...
export default function SponsorCatalog({ delegates, currentSponsor }: Props) {
    ...
    const pool = useMemo(() => {
        ...
        for (const [key, vals] of Object.entries(activeFilters)) {
            ...
            result = result.filter(d => {
                const v = (d.profile as unknown as Record<string, unknown>)[key];
    ...
    const postRequest = useCallback(
        (targetId: string, rank: number) => {
            fetch('/api/requests', {
                ...
                body: JSON.stringify({ requesterId: currentSponsor.id, targetId, rank }),
            }).catch(console.error);
        },
        [currentSponsor.id],
    );
    ...
    const handleApplyFilter = useCallback(
        (id: string, val: string, checked: boolean, type: 'single' | 'multi') => {
        ...
        if (isPicking) {
            return (
                <div className="rank-picker">
                    <div className="rank-picker-label">Select interest level</div>
                    ...
                    <button className="rank-pip-cancel" onClick={() => setPickingId(null)}>
                        Cancel
                    </button>
                </div>
            );
    ...
    function renderGrid() {
        if (!pool.length)
            return <div className="no-results">No delegates match your filters.</div>;
        return (
                ...
                {pool.map(d => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <CardMoreToggle p={p} />
                            <div className="card-action">{renderCardAction(d)}</div>
                        </div>
                    );
                })}
            </div>
                        ...
                        return (
                            ...
                            >
                                <div className="list-cell">
                                    ...
                                    <div className="list-action-cell">{action}</div>
                                </div>
                                <div className="list-cell">
                                    <div className="lc-company">{d.company}</div>
                                </div>
                                <div className="list-cell">
                                    <span className={`${rc} rev-chip`}>{str(p.annualRevenue)}</span>
                                </div>
                                ...
                                <div className="list-cell">
                                    <span className="lc-tags">{dotTags(p.areasOfSpecialization)}</span>
                                </div>
                                <div className="list-cell">
                                    <span className="lc-tags">{dotTags(p.industrySectors)}</span>
                                </div>
                                <div className="list-cell">
                                    <span className="lc-tags">{dotTags(p.regionsOverseen)}</span>
                                </div>
                                <div className="list-cell">
                                    <span className="lc-tags">{dotTags(p.strategicPriorities)}</span>
                                </div>
                            </div>
                        );
                    })}
                    ...
                    if (req !== undefined && !isPicking) {
                        ...
                        action = (
                            <div className="hcard-rank-picker">
                                <span className="hcard-rank-label">Interest level:</span>
                        ...
                        >
                            <div className="hcard-top">
                                <div className="hcard-identity">
                                    <div className="hcard-company">{d.company}</div>
        ...
        <>
            ...
            <header className="topbar">
                ...
                <div className="topbar-right">
                    <div className="user-chip">
                        <div>
                            <div className="user-name">{currentSponsor.name}</div>
                            <div className="user-sub">{currentSponsor.title}</div>
                        </div>
                    </div>
                    <button
                ...
                <div className="catalog-col">
                    <div className="toolbar">
                        ...
                        >
                            ⚙ Filter
                            {totalFilters > 0 && (
                                <span className="filter-btn-badge">{totalFilters}</span>
                            )}
                        </button>
                        ...
                        <div className="sort-wrap">
                            ...
                            >
                                ...
                                <option value="budgetaryResponsibility">Budgetary Resp.</option>
                                <option value="plannedSpend">Planned Spend</option>
                    ...
                    {atCap && (
                        <div className="cap-banner">
                            ...
                            <span>
                                You&apos;ve reached your limit of {maxMeetings} meeting
                                requests. Remove a request to add a new one.
                            </span>
                        </div>
                    )}
                    ...
                    <div ref={catalogRef}>
                        ...
                        {viewMode === 'list' && (showHCards ? renderHCards() : renderList())}
                    </div>
                </div>
            </div>
            ...
            <div className={`drawer ${drawerOpen ? 'open' : ''}`}>
                ...
                <div className="drawer-notice">
                    <strong>Requests are saved automatically.</strong> You can edit interest
                    levels or remove requests at any time before the window closes.
                </div>
                    ...
                    {drawerEntries.length === 0 ? (
                        <div className="drawer-empty">
                            ...
                            <div style={{ fontSize: '12px' }}>
                                Click &quot;+ Request Meeting&quot; on any delegate to get started.
                            </div>
                        </div>
                    ) : (
